...
WriteStream.REQUEST = HTTPParser.REQUEST;
WriteStream.RESPONSE = HTTPParser.RESPONSE;

WriteStream.prototype.setMaxHeaderPairs = function (count) {
  const parser = this.parser;
  // Propagate headers limit from request object to parser
  if (typeof count === 'number') {
    parser.maxHeaderPairs = count << 1;
  } else {
...
WriteStream.prototype.reinitialize = function (type) {
  const parser = this.parser;
...
WriteStream.prototype.free = function (req, socket) {
  const parser = this.parser;
  if (parser) {
    parser._headers = [];
